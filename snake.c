...
void generatefruit(int height, int width, cord * fruit, snake * snake);
...
int validPos(snake * snake, cord * cord);


int main(int argc, char * argv[]) {
    int height, width;
    if (argc < 3) {
	printf("2 Arguments Expected: Height Width\n");
	exit(EXIT_FAILURE);
    }
	...
	printf("HEIGHT and WIDTH must be at least 15\n");
	exit(EXIT_FAILURE);
    }
...
void startgame(int height, int width) {
    ...
    generatefruit(height, width, &fruit, snake);
    dir = UP;
    gameover = 0;
    score = 0;
	...
	gameover = !validPos(snake, snake->lastAdded->yx);	
	refresh();
    }
...
int validPos(snake * snake, cord * cord) {
    queue *curr = snake->firstAdded;
    while (curr != snake->lastAdded) {
	if (curr->yx->y == cord->y && curr->yx->x == cord->x) return 0;
	curr = curr->next;
    }
    return 1;
}
